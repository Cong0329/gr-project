...
export const ChatModal: React.FC<ChatModalProps> = ({
    ...
    const handleSendMessage = () => {
        ...
        setInputMessage('');
        dispatch(sendMessageDoctor({ content: inputMessage, recipientId: id, id: user?.id }));
    };
                    ...
                    {messages.map((message) => (
                        ...
                        >
                            <div className={`flex items-start space-x-2 max-w-xs ${message.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm ${message.sender === 'user' ? 'bg-blue-500' : 'bg-gray-500'
                                    }`}>
