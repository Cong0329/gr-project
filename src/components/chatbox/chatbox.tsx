...
import { fetchMessagesUser, sendMessageUser } from '../../redux/messageAsyncThunk';
                ...
                <div className="flex-1 p-3 overflow-y-auto">
                  {messages.map((msg, index) => (
                    <div key={msg.id || index} className={`mb-2  ${msg.sender_id === user.id ? 'text-right' : 'text-left'}`}>
                      ...
                      {msg.content && (
                        <span className={`inline-block px-4 py-2 rounded-lg ${msg.sender_id === user.id ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'}`}>
                          {msg.content}
                        </span>
                      )}
