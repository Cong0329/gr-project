...
export const CartSummartSkeleton: React.FC<CartSummartSkeletonProps> = ({ isCheckout, isDetail }) => {
    return (
        <div className={`bg-white p-4 rounded-lg animate-pulse ${isCheckout ? "mt-7" : "mt-0"}`}>
            ...
            <Skeleton className="bg-gray-200 rounded-lg w-full h-12"></Skeleton>
            ...
            <div className="flex justify-between mt-2">
                <div className="flex items-center gap-1">
                    ...
                    <Skeleton className="rounded-full bg-gray-200 w-3.5 h-3.5"></Skeleton>
                </div>
                <Skeleton className="h-5 w-20 bg-gray-200 rounded"></Skeleton>
            </div>
            ...
            <div className="flex justify-between mt-2 pt-2 border-t border-gray-300">
                <Skeleton className="h-6 w-24 bg-gray-200 rounded"></Skeleton>
                <Skeleton className="h-6 w-28 bg-gray-200 rounded"></Skeleton>
            </div>

            {/* Payment method skeleton */}
            {isDetail && (
                <div className="mt-2 border-t pt-2">
                    <Skeleton className="h-5 w-40 bg-gray-200 rounded"></Skeleton>
                    <div className="flex items-center mt-2 gap-2">
                        <Skeleton className="h-10 w-10 bg-gray-200 rounded-md"></Skeleton>
                        <Skeleton className="h-5 w-56 bg-gray-200 rounded"></Skeleton>
                    </div>
                </div>
            )}

            {/* Button skeleton */}
            <Skeleton className="w-full mt-4 h-10 bg-gray-200 rounded-full"></Skeleton>
        </div>
    )
}