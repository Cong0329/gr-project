...
export const VoucherModal = ({ onClose }: VoucherModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 tb:hidden">
            <div className="bg-white rounded-xl shadow-lg w-1/3 md-lg:w-1/2">
                {/* Header */}
                ...
                <div className="text-center mb-4 overflow-y-auto max-h-64   bg-gray-200 scrollbar-hide">
                    <div className="bg-white p-2">
                        <div className="flex items-center rounded-lg gap-2 w-full border">
                            <input ref={inputRef} type="text" className="focus:outline-none w-4/5 my-2 ml-2" placeholder="Nhập mã giảm giá" />
                            <button onClick={handleApply} className="w-1/5 py-2 bg-gray-200 cursor-not-allowed rounded-r-lg border">Xác nhận</button>
                        </div>
                    </div>
                ...
                <div className="flex flex-col justify-end gap-2 font-semibold px-4 pb-4">
                    ...
                    <button onClick={handleApply} className="px-4 py-2 bg-blue-700 text-white rounded-full w-full">Áp dụng</button>
                </div>
            </div>
            <Sheet isOpen={true} onClose={onClose} className="tb:block bg-black bg-opacity-50 hidden">
                <Sheet.Container className="max-h-[420px] overflow-hidden rounded-lg">
                    <div className="flex items-center justify-between font-bold text-lg border-b p-4">
                        <p className="text-center grow">Ưu đãi dành cho bạn</p>
                        <button onClick={onClose} className="flex-none self-start"><FaXmark /></button>
                    </div>
                    <Sheet.Content className="bg-white max-h-full overflow-y-auto">
                        <div className="text-center mb-4 overflow-y-auto max-h-64 bg-gray-200 scrollbar-hide">
                            <div className="bg-white p-2">
                                <div className="flex items-center rounded-lg gap-2 w-full border">
                                    <input ref={inputRef} type="text" className="focus:outline-none w-4/5 my-2 ml-2" placeholder="Nhập mã giảm giá" />
